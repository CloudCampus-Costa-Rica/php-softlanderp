...
class FacturaReclasifHandler extends SoftlandHandler
{
    ...
    public function insertarDiario($documento, $cliente, $impuestos, $asiento, $pdo = null)
    {
        ...
        // comentamos esta linea para oin incluir el mov de impuesto ya que esta dado en el primer asiento
        /*if ($documento->impuesto > 0) {
            $ln[] = "i";
        }*/

        $global = 1;
        // insertar primera linea debito para la cuenta contable aux
        $linea = new Diario();
        $linea->asiento = $asiento;
        $linea->consecutivo = $global++;
        $linea->nit = $cliente->nit;
        $linea->centroCosto = $documento->centroCostoAux;
        $linea->cuentaContable = $documento->cuentaContableAux;
